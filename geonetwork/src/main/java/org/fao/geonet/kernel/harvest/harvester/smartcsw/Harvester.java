...
class Harvester
{
	...
	public Harvester(Logger log, ServiceContext context, Dbms dbms, SmartCswParams params)
	{
		this.log    = log;
	...
	public SmartCswResult harvest() throws Exception
	{
		...
		if (params.isSearchEmpty()) {
			log.warning("Must specify XPath for smart parsing");
			return null;
		}
	...
	private Set<RecordInfo> search(CswServer server, Search s) throws Exception
	{
		...
		while (true)
		{
			...
			for (Object e :list)
			{
				...
				RecordInfo recInfo = getRecordInfo(record, xpath);
			...
			// start > 2000 just to stop before 400000
			if (start+GETRECORDS_NUMBER_OF_RESULTS_PER_PAGE > recCount || start > 2000)
				break;
	...
	/**
	 * getRecordInfo is called on each record to get the important information
	 * @param record element representing one record in the results
	 * @param urlXpath user specified XPath of location of endpoints within output
	 * schema of given CSW response.  This is used to build up the Endpoint objects
	 * which determines whether a record contains the necessary information.
	 * @return RecordInfo object if proper endpoints were found, null otherwise
	 */
	private RecordInfo getRecordInfo(Element record, XPath urlXpath) {
		...
		if ((name.equals("SummaryRecord") || (name.equals("Record")))) {
			...
			try {
				...
				xpath = XPath.newInstance("dct:modified");
				Element modified = (Element) xpath.selectSingleNode(record);
				//String modified = record.getChildText("modified",   dct);
				List elements = urlXpath.selectNodes(record);

				boolean hasValidEndpoint = false;
				for (Object ele : elements) {
					Endpoint endpoint = new Endpoint(((Element) ele).getText());
					// if endpoint is wms or unmatched it is not sufficient for admittance
					if (endpoint.getType() != EndpointType.UNKNOWN && endpoint.getType() != EndpointType.WMS) {
				...
				if (!hasValidEndpoint) {
					log.warning("Skipped record with no valid endpoints : " + name);
					return null;
				}
				...
				log.warning("Error parsing metadata: " + dome);
				return null;
			}
            ...
            try {
				...
				List elements = urlXpath.selectNodes(record);
				for (Object ele : elements) {
					log.warning(((Element)ele).getText());
					Endpoint endpoint = new Endpoint(((Element)ele).getText());
					// if endpoint is wms or unmatched it is not sufficient for admittance
					if (endpoint.getType() != EndpointType.UNKNOWN && endpoint.getType() != EndpointType.WMS) {
						hasValidEndpoint = true;
	...
	public static final String PREFERRED_HTTP_METHOD = CatalogRequest.Method.POST.toString();
	private static int GETRECORDS_NUMBER_OF_RESULTS_PER_PAGE = 50;
	private static String CONSTRAINT_LANGUAGE_VERSION = "1.1.0";
	private Logger         log;
	private Dbms           dbms;
	private SmartCswParams params;
