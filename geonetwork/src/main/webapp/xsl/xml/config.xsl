...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/system">
		...
		<xsl:variable name="smartcsw"        select="children/smartcsw/children"/>
		<xsl:variable name="smartcswInfo"    select="$smartcsw/contactInfo/children"/>
		<xsl:variable name="smartcswPhone"   select="$smartcswInfo/phone/children"/>
		<xsl:variable name="smartcswAddress" select="$smartcswInfo/address/children"/>
		...
		<config>
			...
			<smartcsw>
				<enable><xsl:value-of select="$smartcsw/enable/value"/></enable>
				<contactId><xsl:value-of select="$smartcsw/contactId/value"/></contactId>
				<individualName><xsl:value-of select="$smartcsw/individualName/value"/></individualName>
				<positionName><xsl:value-of select="$smartcsw/positionName/value"/></positionName>
				<contactInfo>
					<phone>
						<voice><xsl:value-of select="$smartcswPhone/voice/value"/></voice>
						<facsimile><xsl:value-of select="$smartcswPhone/facsimile/value"/></facsimile>
					</phone>
					<address>
						<deliveryPoint><xsl:value-of select="$smartcswAddress/deliveryPoint/value"/></deliveryPoint>
						<city><xsl:value-of select="$smartcswAddress/city/value"/></city>
						<administrativeArea><xsl:value-of select="$smartcswAddress/administrativeArea/value"/></administrativeArea>
						<postalCode><xsl:value-of select="$smartcswAddress/postalCode/value"/></postalCode>
						<country><xsl:value-of select="$smartcswAddress/country/value"/></country>
						<email><xsl:value-of select="$smartcswAddress/email/value"/></email>
					</address>
					<hoursOfService><xsl:value-of select="$smartcswInfo/hoursOfService/value"/></hoursOfService>
					<contactInstructions><xsl:value-of select="$smartcswInfo/contactInstructions/value"/></contactInstructions>
				</contactInfo>
				<role><xsl:value-of select="$smartcsw/role/value"/></role>
				<title><xsl:value-of select="$smartcsw/title/value"/></title>
				<abstract><xsl:value-of select="$smartcsw/abstract/value"/></abstract>
				<fees><xsl:value-of select="$smartcsw/fees/value"/></fees>
				<accessConstraints><xsl:value-of select="$smartcsw/accessConstraints/value"/></accessConstraints>
                <metadataPublic><xsl:value-of select="$smartcsw/metadataPublic/value"/></metadataPublic>
            </smartcsw>
