...
//			}
            ...
            // TODO:  CHECK THIS
            // START HACK? THREAD SAFE?
            StatusType status = StatusType.Factory.newInstance();
            status.addNewProcessFailed().addNewExceptionReport().addNewException().
                    addNewExceptionText().setStringValue(e.getMessage());
        ...
        if (algorithm == null || (algorithm.getErrors() != null && algorithm.getErrors().size() > 0)) {
            LOGGER.info("Error while executing the embedded process for: " + getAlgorithmIdentifier());
            StatusType status = StatusType.Factory.newInstance();
            status.addNewProcessFailed().addNewExceptionReport().addNewException().
                    addNewExceptionText().setStringValue(
                    (algorithm == null || algorithm.getErrors() == null) ?
                        "UNKNOWN ERROR" : algorithm.getErrors().get(0));
            this.getExecuteResponseBuilder().setStatus(status);
        } else {
