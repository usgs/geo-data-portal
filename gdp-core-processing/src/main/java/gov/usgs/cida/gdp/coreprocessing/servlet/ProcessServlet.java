...
public class ProcessServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("submitforprocessing".equals(command)) {
            ...
            if (lat != null && lon != null) {
                ...
                // When getting the geometry from the epa nhd service, there won't be
                // any attributes.  But the attribute cannot be null, so set it to a
                ...
                try {
                    featureCollection = NHDService.getGeometry(lon, lat, shapefilePath, shxFilePath);
                } catch (Exception ex) {
                ...
                try {
                    featureCollection = ShapefileToFeatureCollection.shapefileToFeatureCollection(uploadDirectory, outputFile, userDirectory, userspacePath, appTempDir, shapeSet, features, attribute);
            ...
            if ("WCS".equals(dataSetInterface)) {
                File wcsRequestOutputFile = wcs(wcsServer,
                        wcsCoverage, wcsBoundingBox, wcsGridCRS,
                        wcsGridOffsets, wcsResmapleFilter, wcsResampleFactor);
            ...
            if (toTime == null || fromTime == null) {
                ...
                try {
                    ...
                    parsedDates = false;
                    LoggerFactory.getLogger(ProcessServlet.class).debug(e1.getMessage());
                }
            }
            ...
            if (featureDataset.getFeatureType() == FeatureType.GRID && featureDataset instanceof GridDataset) {
                try {
                    grid(featureDataset, categorical, featureCollection, attributeName, delimiterOption, fromDate, toDate, dataTypes, groupById, outputStats, outputFile);
                } catch (Exception ex) {
                    XmlReplyBean xmlOutput = new XmlReplyBean(AckBean.ACK_FAIL, new ErrorBean(ex.getMessage()));
                    XmlUtils.sendXml(xmlOutput, start, response);
                    return;
                ...
                try {
                    station(featureDataset, featureCollection, fromDate, toDate, delimiterOption, dataTypes, groupById, outputFile);
                } catch (Exception ex) {
                    XmlReplyBean xmlOutput = new XmlReplyBean(AckBean.ACK_FAIL, new ErrorBean(ex.getMessage()));
                    XmlUtils.sendXml(xmlOutput, start, response);
                    return;
            ...
            if (!outputDataFile.exists()) {
                XmlReplyBean xmlOutput = new XmlReplyBean(AckBean.ACK_FAIL, new ErrorBean("Unable to create output file."));
                XmlUtils.sendXml(xmlOutput, start, response);
                return;
            }
    ...
    private void grid(FeatureDataset featureDataset, boolean categorical,
            FeatureCollection<SimpleFeatureType, SimpleFeature> featureCollection,
            String attributeName, DelimiterOption delimiterOption, Date fromDate,
            Date toDate, String[] dataTypes, String groupById, String[] outputStats,
            String outputFile)
            throws IOException, SchemaException, TransformException, IllegalArgumentException, FactoryException, InvalidRangeException {
        ...
        if (categorical) {
            ...
            try {
                ...
                // *** long running task ***
                FeatureCategoricalGridCoverage.execute(featureCollection, attributeName, gridDataset, gridName, writer, delimiterOption.delimiter);
            } finally {
                if (writer != null) {
                    try {
                        writer.close();
            ...
            try {
                ...
                try {
                    ...
                    LoggerFactory.getLogger(ProcessServlet.class).error(e.getMessage());
                ...
                try {
                    writer = new BufferedWriter(new FileWriter(new File(System.getProperty("applicationWorkDir"), outputFile)));
                    // *** long running task ***
                    FeatureCoverageWeightedGridStatistics.execute(featureCollection, attributeName, gridDataset, gridName, timeRange, statisticList, writer, groupBy == GroupBy.GridOption.statistics, delimiterOption.delimiter);
                } finally {
                    if (writer != null) {
                        try {
                            writer.close();
    ...
    private void station(FeatureDataset featureDataset, FeatureCollection<SimpleFeatureType, SimpleFeature> featureCollection,
            Date fromDate, Date toDate, DelimiterOption delimiterOption, String[] dataTypes,
            String groupById, String outputFile)
            throws FactoryException, SchemaException, org.opengis.coverage.grid.InvalidRangeException, TransformException, IOException {
        ...
        if (fcl != null && fcl.size() == 1) {
            ...
            if (fc != null && fc instanceof StationTimeSeriesFeatureCollection) {
                ...
                try {
                    writer = new BufferedWriter(new FileWriter(new File(System.getProperty("applicationWorkDir"), outputFile)));
                    StationDataCSVWriter.write(featureCollection, stsfc, variableList, new DateRange(fromDate, toDate), writer, groupBy == StationOption.variable, delimiterOption.delimiter);
                } finally {
                    if (writer != null) {
                        try {
                            writer.close();
    ...
    private File wcs(String wcsServer, String wcsCoverage, String wcsBoundingBox,
            String wcsGridCRS, String wcsGridOffsets, String wcsResmapleFilter,
            String wcsResampleFactor)
        ...
        File wcsRequestOutputFile = new File(System.getProperty("applicationWorkDir"), Integer.toHexString(wcsRequestHashCode) + ".tiff");
        FileOutputStream fos = null;
        InputStream coverageIStream = null;
        try {
            // Create WCS request
            String getCoverageRequest = wcsServer + "?service=WCS" + "&version=1.1.1" + "&request=GetCoverage" + "&identifier=" + wcsCoverage + "&boundingBox=" + wcsBoundingBox + "," + wcsGridCRS + "&gridBaseCRS=" + wcsGridCRS + "&gridOffsets=" + wcsGridOffsets + "&format=image/GeoTIFF";
            // Call getCoverage
            HttpURLConnection httpConnection = HTTPUtils.openHttpConnection(new URL(getCoverageRequest), "GET");
            ...
            if (boundaryString == null) {
                throw new IOException(); // TODO: probably change exception thrown
            }
            int part = 0;
            ...
            while ((bytesRead = coverageIStream.read(b, 0, 1024)) > 0) {
                String endString = new String(Arrays.copyOfRange(b, bytesRead - closeTag.length(), bytesRead));
