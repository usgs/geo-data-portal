...
public abstract class AnnotationBinding<M extends AccessibleObject & Member> {
    ...
    public static class ProcessMethodBinding extends AnnotationBinding<Method> {

        public ProcessMethodBinding(Method method) {
            super(method);
        }
        ...
        public boolean validate() {
            if (!checkModifier()) {
                LOGGER.error("Method {} with Process annotation can't be used, not public.", getMember());
                return false;
            }
            // eh, do we really need to care about this?
            if (!getMember().getReturnType().equals(void.class)) {
                LOGGER.error("Method {} with Process annotation can't be used, return type not void", getMember());
                return false;
            }
            if (getMember().getParameterTypes().length != 0) {
                LOGGER.error("Method {} with Process annotation can't be used, method parameter count is > 0.", getMember());
                return false;
            }
            return true;
        }
    ...
    public static abstract class OutputBinding<M extends AccessibleObject & Member,  D extends OutputDescriptor> extends DataBinding<M,D> {
        ...
        protected boolean checkType( ) {
            ...
            try {
                ...
                if (bindingPayloadType instanceof Class<?>) {
                    ...
                    return bindingClass.getConstructor(bindingPayloadClass) != null &&
                           bindingPayloadClass.isAssignableFrom(inputPayloadClass);
                }
            } catch (NoSuchMethodException e) {
        ...
        public IData bindOutputValue(Object outputValue) {
            try {
                Class<? extends IData> bindingClass = getDescriptor().getBinding();
                Constructor<? extends IData> bindingConstructor = bindingClass.getConstructor(outputValue.getClass());
                if (isTypeEnum()) {
