...
import gov.usgs.cida.n52.wps.algorithm.annotation.AnnotatedAlgorithmIntrospector;
import static gov.usgs.cida.n52.wps.algorithm.annotation.AnnotatedAlgorithmIntrospector.getInstrospector;
import gov.usgs.cida.n52.wps.algorithm.annotation.AnnotationBinding;
...
public abstract class AbstractAnnotatedAlgorithm extends AbstractDescriptorAlgorithm {
    ...
    public Map<String, IData> run(Map<String, List<IData>> inputMap) {
        // I plan on  separating the implementation from the annotated
        // process/algorithm, there's no reason it needs to me a subclass
        // (simply that way now for development convenience)
        ...
        getInstrospector(annotatedInstance.getClass()).getProcessMethodBinding().execute(annotatedInstance);
